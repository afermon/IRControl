...
namespace ircontrol
{
    public partial class FormMain : Form
    {
        ...
        private bool _notfoundcmd = false;
        ...
        public void InCmd( string incmd)
        {
            tb_event.Text = tb_event.Text + "\r\nCode >" + incmd + "<";
            tb_event.SelectionStart = tb_event.Text.Length;
            tb_event.ScrollToCaret();
               ...
               try
                {
                    ...
                    xmlconfigdocument.Load(_xmlconfigfile);
                    XmlNodeList fgIrcontrol = xmlconfigdocument.GetElementsByTagName("fg_ircontrol");
                    XmlNodeList lista = ((XmlElement)fgIrcontrol[0]).GetElementsByTagName("code");
                    foreach (XmlElement nodo in lista)
                    {
                        int i = 0;
                        XmlNodeList description = nodo.GetElementsByTagName("description");
                        XmlNodeList hexCode = nodo.GetElementsByTagName("hexcode");
                        XmlNodeList sendKeyCmd = nodo.GetElementsByTagName("sendkeycmd");
                        if (code == hexCode[i].InnerText)
                            {
                                    VirtualKeySend(sendKeyCmd[i].InnerText.Trim()); 
                                    tb_event.Text = tb_event.Text + "\r\nSend >" + description[i].InnerText;
                                    tb_event.SelectionStart = tb_event.Text.Length;
                                    tb_event.ScrollToCaret();
                                    _notfoundcmd = false;
                            }
        ...
        private void BtnOpenxmlClick(object sender, EventArgs e)
        {
            DialogResult result = ofd_xmlconfig.ShowDialog();
            if (result == DialogResult.OK)
            {
                _xmlconfigfile = ofd_xmlconfig.FileName;
                _configfile = true;
                tb_event.Text = tb_event.Text + "\r\nXML >"+ _xmlconfigfile;
        ...
        public void VirtualKeySend(string codigo)
        {
            ...
            inputSimulator.Keyboard.KeyPress(VirtualKeyCode.MENU, VirtualKeyCode.ADD);
            switch (codigo)
            {
